...
Introduced by commit `d14b3716` (`feat(server): add MCP git plugin resolver with security hardening`). The resolver is a standalone module that lets MCP server plugins be sourced directly from git repositories (public HTTPS, private SSH, local `file://`). No caller currently consumes it — wiring into `mcp-servers.json` resolution and an `alexi mcp install` command is planned as follow-up. When touching this surface or adding a caller:

- **Do NOT bypass `parseGitUrl` when passing values to `git`.** Every clone / `rev-parse` / `ls-remote` invocation MUST use `parsed.normalizedUrl`, never the caller-supplied raw string. The parser strips the `#<ref>@<subpath>` fragment, refuses unrecognised schemes, and refuses subpaths that start with `/`, `~`, or contain `..`. Reintroducing the raw URL to any child-process argv reopens the option-injection surface the resolver was designed to close.
- **`validateRef` is the security boundary, not a sanity check.** It rejects any ref that starts with `-` (option injection), that is empty or whitespace-only, that starts with `.` or ends with `.lock`, that contains `..`, `@{`, control chars, whitespace, or `~`, `^`, `:`, `?`, `*`, `[`, `\`. When adding a new entry point that accepts a ref from user / config input, call `validateRef(ref)` BEFORE any shell-out — never trust a ref that has only been fragment-parsed off a URL. Both `cloneGitRepo` and `revalidateMutableRef` already do this; if you introduce a third entry point, mirror the pattern and add a pre-flight rejection test (`await expect(...).rejects.toThrow(/parsed as an option/)`) alongside the happy-path coverage.
- **Preserve the atomic-swap semantics of `cloneGitRepo`.** The clone runs into `<cacheDir>/<identity>.tmp-<pid>-<random>` and only becomes visible under the final identity path via `fs.renameSync`. Never write directly into `<cacheDir>/<identity>/` — that would let a partial clone be observed as complete. Failed clones MUST clean up any surviving `.tmp-` sibling best-effort so the plugin cache does not accumulate garbage; the current implementation swallows a secondary cleanup failure so the ORIGINAL clone error is what surfaces to the caller.
- **Always run `checkContainment` before reading a subpath.** A cloned repository is untrusted input. `checkContainment(repoPath, subpath)` resolves `fs.realpathSync` on both the repo root and the joined subpath, then verifies the result is inside the repo. A caller that reads `path.join(clonedRepo, subpath, 'plugin.js')` directly, without the containment check, can be redirected to `/etc/passwd` by a repo that ships `subpath/link -> /etc/passwd`. Non-existent paths are still rejected lexically so `subpath: ../../etc` is refused before any I/O.
- **`getPluginIdentity` is the ONLY correct key for the plugin cache and for `mcp-servers.json` entries.** The identity format is `git-<protocol>-<sha256[..16]>` over `(normalizedUrl, ref, subpath)`. Do NOT hand-derive an identity from any subset of those fields — the same `github.com/org/repo` accessed via HTTPS vs SSH must map to different identities so caller credentials do not leak across protocols, and the same repo pinned to `main` vs `v1.0.0` must map to different identities so revalidating one does not invalidate the other.
- **Respect the mutable-ref revalidation TTL.** `shouldRevalidate(identity, ttl, cacheDir)` is the gate — it returns `true` for missing metadata, malformed JSON (fail-open), and past-TTL entries. Only call `revalidateMutableRef` when the gate says so; otherwise the resolver does one `ls-remote` per plugin resolution and defeats the caching. A full commit SHA (immutable) never needs revalidation at all; the caller is expected to short-circuit on the ref shape before entering the revalidation path. `DEFAULT_REVALIDATE_TTL_MS` is 24 hours; do not lower it globally without profiling — this is a network round-trip per plugin per configured TTL cycle.
- **SSH key handling routes through `GIT_SSH_COMMAND`.** When a caller supplies `sshKey`, `cloneGitRepo` / `revalidateMutableRef` set `GIT_SSH_COMMAND=ssh -i <key> -o StrictHostKeyChecking=accept-new -o IdentitiesOnly=yes` on the child process only. The user's global ssh config is untouched. Do NOT switch to `-o StrictHostKeyChecking=no` (which would silently trust any host key change) or drop `IdentitiesOnly=yes` (which would let an ambient agent-forwarded key be picked up instead of the caller's).
- **Windows drive-less `file://` URLs go through `normalizeFileUrl`.** `file:///C:/Users/x` MUST collapse to `file://C:/Users/x` before any filesystem operation, and POSIX `file:///abs/path` MUST pass through unchanged. `parseGitUrl` already applies the normalisation; do not add a second normalisation pass in a caller.
- **Tests belong in `src/mcp/__tests__/git-resolver.test.ts`.** The suite mocks `child_process.execFile` at the module boundary via `vi.mock('child_process', () => ({ execFile: vi.fn() }))` and runs without a real `git` binary. When adding a new entry point that shells out to git, follow the `programExecFile(fixture)` helper pattern documented in [`docs/TESTING.md`](TESTING.md#testing-the-mcp-git-plugin-resolver-srcmcp__tests__git-resolvertestts) — a `handler(call)` that routes on `args[0]` plus a `calls` log to assert on argv shape and env vars after the fact. Materialise the temp directory from inside the fixture handler on the `clone` case; the module runs `git rev-parse HEAD` with `{ cwd: tmpPath }` immediately after clone and needs the dir to exist. Use `fs.mkdtempSync` per test rather than sharing `PLUGIN_CACHE_ROOT`, and cover the pre-shell-out `validateRef` rejection separately from the happy path so a regression that moves the check into `execFileAsync` still trips the suite.
