...
The 2026-09-22 sync (commit `82c31d07`, `1.22.27` → `1.22.28`) is a worked example of a high-touch sync where every touched file is a **new** module — no existing module was overwritten, no downstream import chain was broken, and every new export is documented in the CHANGELOG `### Added` / `### Changed` blocks with its upstream commit reference. Six new modules landed under `src/` (`core/npm.ts`, `permission/mcp-metadata.ts`, `permission/reply-retry.ts`, `providers/cloudflare-ai-gateway.ts`, `providers/enabled-filter.ts`) plus one edit to `src/compaction/index.ts` (SUMMARY_PROMPT rewrite) and one edit to `src/core/session/retry.ts` (broadened transient-network regex list). When reviewing sync PRs that add new modules, verify each new file is (a) a self-contained helper with an inline JSDoc block naming the upstream commit(s) it ports, (b) safe by construction — no side effects at import time, no top-level auth loads — and (c) accompanied by a CHANGELOG entry that lists every new public export by name.
