...
**When changing the provider `TokenUsage` shape or the reasoning-token extraction path**, mock `@sap-ai-sdk/orchestration` at the module boundary (both `chatCompletion` and `stream`) and route both call-site assertions through the mocked SDK — do NOT mock `normalizeTokenUsage` itself, or the test will pass even after a regression that reverts `SapOrchestrationProvider.complete()` / `.stream()` to the pre-2026-09-26 inline shape. Assert BOTH `reasoningTokenCount` AND the reduced `completion_tokens` on every case so a regression that stops subtracting is caught. See [`docs/TESTING.md#testing-reasoning-token-accounting-testsproviderssapOrchestration-reasoningTokenstestts`](./TESTING.md#testing-reasoning-token-accounting-testsproviderssapOrchestration-reasoningTokenstestts) for the reference suite and [`docs/PROVIDERS.md#reasoning-token-accounting-issue-1844`](./PROVIDERS.md#reasoning-token-accounting-issue-1844) for the runtime contract.
