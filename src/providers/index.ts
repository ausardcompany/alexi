...
export type ProviderType = "proxy" | "native" | "anthropic" | "claude-native" | "orchestration"

export function resolveProvider(): { type: ProviderType; sdk: any } {
  const proxyBase = env("SAP_PROXY_BASE_URL")
  if (proxyBase) return { type: "proxy", sdk: createSapProxyProvider(proxyBase, env("SAP_PROXY_API_KEY")) }
  const nativeKey = env("AICORE_SERVICE_KEY")
  if (nativeKey) return { type: "native", sdk: createSapNativeProvider() }
 ...
 */
function getClaudeDeploymentId(modelId: string): string | null {
  const lower = modelId.toLowerCase()
 ...
 */
export function shouldUseOrchestration(): boolean {
 ...
 */
export function getProviderForModel(modelId: string): { type: ProviderType; sdk: any } {
