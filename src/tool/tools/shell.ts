 ...
 * Enhanced with shell prompt system for improved safety and tracking
 */
...
- Don't expose secrets in command arguments`,
  ...
  async execute(params, context): Promise<ToolResult<ShellResult>> {
    ...
    const execShellId = shellId();

    // Analyze command for safety warnings
    const promptResult = shellPrompt({
      command: params.command,
      workingDirectory: params.workdir,
      timeout: params.timeout,
    });

    // Log safety warnings if present
    if (promptResult.safetyWarnings.length > 0) {
      console.warn(`[Shell ${execShellId}] Safety warnings:`, promptResult.safetyWarnings);
    }
    ...
    return new Promise((resolve) => {
      ...
      const proc = spawn(params.command, {
        shell: true,
        cwd: workdir,
        env: { ...process.env, ...promptResult.environmentOverrides, FORCE_COLOR: '0' },
        windowsHide: true,
        detached: true,
      });
...
// Export shell prompt utilities
export { shellId } from '../shell/id.js';
export { shellPrompt } from '../shell/prompt.js';
