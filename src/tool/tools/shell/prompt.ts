...
import type { ShellInfo, ShellType } from './id.js';
...
function buildUnixPrompt(_type: ShellType, cwd?: string, timeout?: number): string {
...
function buildPowerShellPrompt(cwd?: string, _timeout?: number): string {
...
export function build(shell: ShellInfo, options: PromptOptions = {}): string {
 ...
 */
export function getEnvSetup(shell: ShellInfo): Record<string, string> {
 ...
 */
export function getTimeoutCommand(shell: ShellInfo, seconds: number): string {
  switch (shell.type) {
    case 'powershell':
 ...
 */
export function getKillCommand(shell: ShellInfo, pid: number): string {
  switch (shell.type) {
    case 'powershell':
 ...
 */
export function getErrorHandling(shell: ShellInfo): string {
  switch (shell.type) {
    case 'powershell':
 ...
 */
export function getScriptExtension(shell: ShellInfo): string {
  switch (shell.type) {
    case 'powershell':
      return '.ps1';
    case 'cmd':
 ...
 */
export function getCommentPrefix(shell: ShellInfo): string {
  switch (shell.type) {
    case 'cmd':
      return 'REM';
    default:
 ...
 */
export function escapeString(shell: ShellInfo, str: string): string {
  switch (shell.type) {
    case 'powershell':
