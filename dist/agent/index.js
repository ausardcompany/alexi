...
function createAgent(config) {
    return {
        ...
        canUseTool(toolId) {
            ...
            if (this.disabledTools?.includes(toolId))
                return false;
...
export function switchAgent(idOrAlias, reason) {
    return getAgentRegistry().switchTo(idOrAlias, reason);
}
export function parseAgentSwitch(message) {
 ...
 */
export function parseAgentMention(message) {
    const match = message.match(/^@(\w+)\s*(.*)$/s);
    if (!match) {
