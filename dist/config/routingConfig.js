 ...
 */
export function evaluateRule(rule, classification, promptLength, prompt) {
    const { condition } = rule;
    // Check length constraints
    if (condition.minLength && promptLength < condition.minLength)
        return false;
    if (condition.maxLength && promptLength > condition.maxLength)
        return false;
    // Check task types
    if (condition.taskTypes && !condition.taskTypes.includes(classification.type))
        return false;
    // Check complexity
 ...
 */
export function findMatchingRule(rules, classification, prompt) {
    // Filter matching rules
    const matching = rules.filter(rule => evaluateRule(rule, classification, prompt.length, prompt));
