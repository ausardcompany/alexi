...
- Standardized code formatting in `src/permission/allow-everything.ts`: collapsed multi-line function signatures and conditional expressions to single lines per Prettier conventions
- Standardized code formatting in `src/tool/background-process.ts`: collapsed multi-line return type annotation to single line for consistency
- Removed trailing blank line in `src/core/filesystem.ts`
