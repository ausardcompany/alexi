...
- **Prettier auto-fix pass on two `src/core/` files** (commit `86771fc6`, `style(ci): auto-fix lint/format issues [alexi-bot]`, 2026-07-21): The CI auto-fix workflow ran Prettier across the working tree and normalised two files to the repository's Prettier configuration (`singleQuote: true`, `trailingComma: 'es5'`). In `src/core/pty.node.ts` the platform check literal `"win32"` was converted to `'win32'` and a trailing comma was added after the `onData` method definition inside the returned `Proc` object literal. In `src/core/session.ts` the three-element string-literal union `['restored', 'snapshots-disabled', 'unavailable']` passed to `Schema.Literals` inside the `SessionRevert` `Schema.Struct` had its double quotes converted to single quotes and the call was reformatted across three lines to stay within the 100-column Prettier `printWidth`. Diff statistics: `2 files changed, 5 insertions(+), 3 deletions(-)`. Pure formatting change with no behavioural, API, CLI-surface, runtime, or type-shape impact: `spawn(file, args, opts)` in `src/core/pty.node.ts` still returns an object with the same `pid: number` and `onData(listener: (data: string) => void): void` shape declared by the `Proc` interface, and the `SessionRevert` schema still validates the same four optional fields (`partID`, `snapshot`, `diff`, `workspace`) with the same three-value literal union on `workspace`. The stub-removal invariant documented in earlier `### Removed` entries continues to apply — both files remain small internal helpers (19 and 13 lines respectively) that are not part of the CLI, provider, tool-registry, event-bus, or Ink TUI surface documented in `docs/ARCHITECTURE.md`, `docs/API.md`, and `docs/PROVIDERS.md`. CI verifies the change via the `format:check` and `lint` jobs. When writing new code that touches these files (or any file in `src/`), run `npm run format` locally to avoid these no-op fix-up commits from the autohealer — see `docs/CONTRIBUTING.md#formatting-prettier` for the canonical Prettier configuration and the full `lint-staged` + Husky commit-time enforcement chain.
