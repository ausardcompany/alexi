...
- **Two upstream-sync orphan stub files added under `src/core/`** (commit `a14eeca6`, same PR — flagged for autohealing cleanup): The daily upstream sync emitted two new scaffold files that are not imported by any module in `src/` or `tests/` and do not participate in Alexi's runtime pipeline. They are documented here so the next autohealing pass can remove them under the stub-removal invariant documented in the earlier `### Removed` entries.
  ...
  - `src/core/package.json` (3-line stub): A nested `{"version": "1.18.3"}` package.json inside the source tree. Node.js will treat any file inside `src/core/` as belonging to this nested package boundary if the module system resolves it, which conflicts with the top-level `"type": "module"` declaration. Nothing imports from `src/core/`'s nested package boundary; the value is also already stale (repository is now at `1.18.4`).
