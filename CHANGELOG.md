...
- **Quote-style normalization on a single upstream-sync orphan stub** (commit `2374caa3`, `style(ci): auto-fix lint/format issues [alexi-bot]`, 2026-07-13, following the upstream sync at commit `c4072215` — `feat(sync): apply upstream changes (2026-07-13)`, version bump `1.17.30` -> `1.17.31`): The CI auto-fix workflow ran ESLint `--fix` and Prettier across the working tree after the daily upstream sync emitted two scaffold files under `src/tool/`. Pure formatting change with no behavioral, API, runtime, or type-shape impact; CI verifies it via the `format:check` and `lint` jobs. Diff statistics: `1 file changed, 5 insertions(+), 5 deletions(-)`. Individual file impact:
  - `src/tool/github-triage.ts` (7-line orphan stub emitted by the same 2026-07-13 sync at commit `c4072215`, blob delta `2e52c97b..39e10a1f`): All ten string literals in the module-scoped `TEAM` object literal — the five keys' value arrays `["kommander", "simonklee"]`, `["Hona", "Brendonovich"]`, `["jlongster", "rekram1-node", "nexxeln", "kitlangton"]`, `["fwang", "MrMushrooooom", "starptech"]`, and `["Hona"]` — were rewritten from double quotes to single quotes (`'kommander'`, `'simonklee'`, `'Hona'`, `'Brendonovich'`, `'jlongster'`, `'rekram1-node'`, `'nexxeln'`, `'kitlangton'`, `'fwang'`, `'MrMushrooooom'`, `'starptech'`) to satisfy Prettier `singleQuote: true`. The file remains a 7-line orphan stub declaring a single module-scoped `const TEAM = { tui: [...], desktop_web: [...], core: [...], inference: [...], windows: [...] } as const;` with no `import` statements, no `export` statement, and no executable function body. The file is broken on multiple axes: (1) the `TEAM` binding is never exported and never read from any module in `src/` or `tests/` — the `@typescript-eslint/no-unused-vars: error` rule flags this as a lint failure that only survives because it is a `const` declaration (which is allowed to be unused when it starts with a lowercase letter under the current ESLint config, and `TEAM` here is upper-case but the ESLint auto-fix run did not delete the declaration); (2) the file has no `import` of `defineTool` from `src/tool/index.ts`, no Zod schema, no `name`, `description`, or `parameters` fields, and no `execute` handler — none of the four elements required by the tool-definition contract documented in `AGENTS.md` and `docs/ARCHITECTURE.md`'s _Tool System_ section is present; (3) `github-triage` is not a registered tool — there is no `github-triage` entry in `src/tool/registry.ts` and no `src/tool/tools/github-triage.ts`; (4) Alexi is a CLI orchestrator for SAP AI Core, not a GitHub-issue triage product — the team names (`tui`, `desktop_web`, `core`, `inference`, `windows`) and the GitHub handles (`kommander`, `simonklee`, `Hona`, `Brendonovich`, `jlongster`, `rekram1-node`, `nexxeln`, `kitlangton`, `fwang`, `MrMushrooooom`, `starptech`) mirror the upstream `.opencode/tool/github-triage.ts` surface used by the opencode project's issue-router bot (see `.github/reports/diff-report-2026-07-13.md` line 187 and `.github/reports/diff-report-2026-05-25.md` line 1340), which has no counterpart in Alexi's tool taxonomy. Nothing in the source tree imports `src/tool/github-triage.ts`. This is a candidate for autohealing removal in a follow-up `fix(ci): remove broken orphan stub files from upstream sync [autohealing]` commit, matching the stub-removal pattern documented in commits `441b4c14`, `63f359da`, `5399f5bf`, `085951af`, and `dff69219` earlier in this changelog. The companion `src/tool/models-api.json.ts` stub also emitted by the same 2026-07-13 sync (a JSON-inside-`.ts` fragment starting with `{ "requesty": { "id": "requesty", "env": ["REQUESTY_API_KEY"], "api": "https://router.requesty.ai/v1", "name": "Requesty", ... } }`) was not touched by this auto-fix pass because its content is malformed TypeScript that Prettier could not repair — it will be handled by the subsequent autohealing removal alongside `github-triage.ts`.
