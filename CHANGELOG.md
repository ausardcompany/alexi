...
- Recall tool for cross-session memory search
  - Search through past conversation sessions to find relevant context
  - Relevance scoring based on keyword density
  - Session limit configuration with default of 10 sessions
  - Option to include or exclude current session from results
  - Returns top 20 most relevant matches sorted by relevance score
- Dynamic tool registration system
  - registerDynamicTool() function for runtime tool registration
  - unregisterDynamicTool() function for runtime tool removal
  - Separate registry for dynamic tools to prevent conflicts with built-in tools
  - Validation to prevent duplicate tool names
- Tool schema type definitions in centralized schema.ts
  ...
  - ToolPermissionLevel enum for permission management
...
- Enhanced edit tool with line number hints
  ...
  - Returns line numbers of replacements in result
  - Improved performance for large files with targeted search regions
- Abort signal support for long-running operations
  - glob tool checks signal before and during directory traversal
  - grep tool checks signal before and between file searches
  - Proper error handling for aborted operations
  - Validation to prevent glob from operating on files instead of directories
...
- Tool registry now maintains separate collections for built-in and dynamic tools
- Tool list() method now returns combined array of built-in and dynamic tools
- Tool schema exports moved to dedicated schema.ts module for better organization

### Fixed

- ESLint naming conflicts in tool schema definitions by using private schema constants with underscore prefixes
- glob tool now validates that path parameter is a directory, not a file
- grep and glob tools properly handle operation cancellation via abort signals
