...
- **Quote-style, `const`-vs-`let`, block-reflow, and trailing-newline normalization across seven upstream-sync orphan stubs** (commit `f65f2dc1`, `style(ci): auto-fix lint/format issues [alexi-bot]`, 2026-07-04, following the upstream sync at commit `6944bf73` — `feat(sync): apply upstream changes (2026-07-04)`): The CI auto-fix workflow ran ESLint `--fix` and Prettier across the working tree after the daily upstream sync inserted seven scaffold blocks. All seven changes are pure formatting (no behavioral, API, runtime, or type-shape impact) and CI verifies them via the `format:check` and `lint` jobs. Diff statistics: `7 files changed, 21 insertions(+), 12 deletions(-)`. None of the seven files is imported by any module in the source tree, and none corresponds to a canonical Alexi surface — the canonical modules are `src/tool/tools/*` (registered via `src/tool/registry.ts`), `src/core/sessionManager.ts`, `src/permission/next.ts` and `src/permission/index.ts`, and `src/core/orchestrator.ts` / `src/core/streamingOrchestrator.ts` / `src/core/router.ts`. Individual file impact:
  - `src/core/cross-spawn-spawner.ts` (9-line orphan stub, diff `12 ++++++++----`): The single import at line 1 (`import { tap as tapStdio, tapped } from "./kilocode/stdio-tap"`) was rewritten to single quotes (`'./kilocode/stdio-tap'`). The two variable declarations at lines 3-8 were converted from `let` to `const` (ESLint `prefer-const`) and Prettier reflowed each from a single long line into a three-line ternary — `const stdout = proc.stdout ? NodeStream.fromReadable({ evaluate: () => tapped(proc, 'stdout') }) : Stream.empty` and the parallel `const stderr = ...`. The two `tapped(proc, "stdout")` / `tapped(proc, "stderr")` string literals were also rewritten to single quotes. A trailing LF newline was appended after the terminal `tapStdio(proc);` call. The file is broken on multiple axes: (1) the imported path `./kilocode/stdio-tap` does not resolve — there is no `src/core/kilocode/` directory in this repository, and the canonical cross-spawn integration lives in `src/tool/tools/bash.ts` via `execa` / native `child_process`, not through an Effect-Schema-wrapped `NodeStream.fromReadable` surface; (2) `proc`, `NodeStream`, `Stream`, and `tapStdio` / `tapped` are used at file scope with no surrounding function, no `import` for `Stream` / `NodeStream`, and no declaration of `proc` — a hard `tsc --noEmit` error; (3) the local import lacks the `.js` suffix required by NodeNext ESM resolution at runtime.
  - `src/core/fs-util.ts` (2-line orphan stub, diff `4 ++--`): Both `yield*` operators were rewritten to `yield * ` (space-separated) — this is Prettier's `arrowParens` / space-around-operator behavior for the generator delegation operator when it appears in a bare statement expression outside a `function*` body, and a trailing LF newline was appended after the second `const stop = ...` declaration. The file is broken on multiple axes: (1) both statements use `yield*` at file scope with no surrounding `function*` — the generator delegation operator is only valid inside a generator function body, so this is a hard parser error; (2) `fs.resolve`, `location.directory`, and `location.project.directory` are undeclared symbols with no `import` in the file; (3) the path `src/core/fs-util.ts` does not correspond to any canonical Alexi filesystem surface — path resolution in Alexi lives in `src/utils/paths.ts` and the temp-directory pattern used by tool tests (`fs.mkdtemp` in `afterEach`), not through an Effect-Schema `fs.resolve(...)` API. Nothing in the source tree imports `src/core/fs-util.ts`.
  - `src/core/session/compaction.ts` (1-line orphan stub, diff `2 +-`): A trailing LF newline was appended after the single statement `compactSession(session, hoistedDependencies);`. Pure whitespace change. The file is broken: (1) `compactSession`, `session`, and `hoistedDependencies` are undeclared at file scope with no `import` — a hard `tsc --noEmit` error; (2) the path `src/core/session/compaction.ts` collides with the canonical compaction surface at `src/compaction/` (`src/compaction/index.ts`, `src/compaction/compaction-chunks.ts`) that is documented in `docs/ARCHITECTURE.md`'s _Compaction_ section and wired into `src/core/agenticChat.ts`. The canonical session model lives in `src/core/sessionManager.ts`, not under `src/core/session/`. Nothing in the source tree imports `src/core/session/compaction.ts`.
  - `src/permission/headless.ts` (31-line orphan stub, diff `8 ++++++--`): Prettier reflowed the single-line Drizzle-ORM query chain at line 22 (`db.select({ parent: SessionTable.parent_id }).from(SessionTable).where(eq(SessionTable.id, id as SessionID)).get()`) into a five-line method-chain block (`db\n  .select({ parent: SessionTable.parent_id })\n  .from(SessionTable)\n  .where(eq(SessionTable.id, id as SessionID))\n  .get()`) per Prettier's `printWidth: 100` policy, and the trailing comma after `.get()` was removed to match the closing `)` on the next line. A trailing LF newline was appended after the closing `}` of the `KiloHeadless` namespace. The file is broken on multiple axes: (1) `Database.use`, `SessionTable.parent_id`, `SessionTable.id`, `eq`, and the `SessionID` type are all undeclared with no `import` in the file — the canonical Alexi permission surface uses `src/permission/next.ts` (`evaluatePermission`, rule matcher, wildcard support) and does not depend on Drizzle ORM or a `SessionTable` schema; (2) the `KiloHeadless` namespace name references the "Kilo" upstream ancestor project (`Kilo-Org/kilocode`), not Alexi's canonical `Permission` namespace exported from `src/permission/index.ts`; (3) headless / non-interactive permission handling in Alexi lives in `src/permission/prompt.ts` and the `--yes` / `--no-prompt` CLI flags, not through a `mark` / `clear` / `denies` root-tracking API backed by a session-parent database lookup. Nothing in the source tree imports `src/permission/headless.ts`.
  - `src/tool/code-mode-integration.test.ts` (6-line orphan stub, diff `2 +-`): A trailing LF newline was appended after the closing `});` of the `describe` block. Pure whitespace change. The file is broken: (1) it uses `describe` and `it` without importing them from `vitest` — a hard `tsc --noEmit` error under the repository's `types: ["vitest/globals"]` policy in `tsconfig.json` (Vitest globals are opt-in and the repository imports the harness explicitly in every canonical test under `tests/` and `src/**/*.test.ts`); (2) the `it` body is a comment-only `// Test implementation` placeholder with no `expect(...)` assertion, so the test contributes nothing to coverage; (3) the file has no corresponding source module — there is no `src/tool/code-mode.ts` canonical implementation to integration-test (see the entry below). Nothing in the test runner picks this file up for coverage.
  - `src/tool/code-mode.ts` (5-line orphan stub, diff `2 +-`): A trailing LF newline was appended after the closing `});` of the `Tool.define` call. Pure whitespace change. The file is broken on multiple axes: (1) `Tool.define` is not a canonical Alexi API — the canonical tool-definition helper is `defineTool` from `src/tool/index.ts`, used with a Zod schema and registered via `src/tool/registry.ts` (see `docs/API.md`'s _Tool System_ section and `AGENTS.md`'s tool authoring rules); (2) the `Tool` namespace is undeclared with no `import` in the file — a hard `tsc --noEmit` error; (3) the `execute: async (params) => { /* Code mode execution logic */ }` body is a comment-only placeholder with no schema, no permission action, and no return value; (4) there is no "code mode" tool in Alexi's canonical tool set — the built-in tools registered under `src/tool/tools/` are `read`, `write`, `edit`, `glob`, `grep`, `bash`, `task`, `webfetch`, `notebook`, `skill`, `todowrite`, and the other tools listed in `docs/API.md`'s _Built-in Tools_ table, and none is named `code-mode`. Nothing in the source tree imports `src/tool/code-mode.ts`.
  - `src/tool/plan.ts` (4-line orphan stub, diff `3 ++-`): The single `yield*` operator on line 3 was rewritten to `yield * ` (space-separated) per Prettier's operator-spacing policy, and Prettier reflowed the `const file = yield* Effect.promise(...)` declaration from a single long line into a two-line assignment (`const file =\n  yield * Effect.promise(() => PlanFile.locate(target, messages, info, instance, ctx.agent));`) to fit within `printWidth: 100`. A trailing LF newline was appended after the closing `);`. The file is broken on multiple axes: (1) all three statements use `PlanFile.resolve`, `Session.plan`, `Effect.promise`, and `PlanFile.locate` at file scope with no `import` — hard `tsc --noEmit` errors; (2) `yield*` is used at file scope with no surrounding `function*` — a hard parser error; (3) `params`, `instance`, `info`, `messages`, and `ctx.agent` are undeclared free variables; (4) the path `src/tool/plan.ts` collides with the canonical planning surface at `src/plan/` (`src/plan/index.ts`, planning-mode entry points) and the canonical plan tool would live at `src/tool/tools/plan.ts` and be registered via `src/tool/registry.ts` — neither `src/tool/tools/plan.ts` nor a canonical `plan` tool registration currently exists (the planning system in Alexi is invoked via the `plan` agent mode declared in `src/agent/index.ts`, not through a `plan` tool). Nothing in the source tree imports `src/tool/plan.ts`.
- All seven files match the upstream-sync orphan pattern documented repeatedly in earlier `### Changed` and `### Removed` entries (commits `f4aa0be1`, `2dd6f396`, `6dc4b883`, `47005135`, `085951af`, `dff69219`, and others). They are candidates for autohealing removal in a follow-up `fix(ci): remove broken orphan stub files from upstream sync [autohealing]` commit. The architectural invariant remains unchanged: canonical tool implementations live exclusively under `src/tool/tools/` and are registered via `src/tool/registry.ts`; canonical session, permission, compaction, and cross-spawn surfaces live at the paths documented in `docs/ARCHITECTURE.md`, not at the sibling paths where the daily upstream sync deposits scaffolds. `package.json` version (`1.17.27` at line 3), `dependencies`, `devDependencies`, `engines.node` (`>=22.12.0`), `bin` (`alexi`, `ax`), and `scripts` are all unchanged; no test coverage impact; no CLI-surface impact.
